...
@NoArgsConstructor
@AllArgsConstructor
public class CentreUpdateRequest {
    ...
    public static Centre from(CentreUpdateRequest request){
        return Centre.builder().id(request.getId()).name(request.getName()).type(CentreType.valueOf(request.getType()).name()).phone(request.getPhone()).
                description(request.getDescription()).uri(request.getUri()).
