...
public class VenueUpdateRequest {
    ...
    @Positive(message = "ID is mandatory for updates")
    private Integer id;
    ...
    public static Venue from(VenueUpdateRequest request,Integer countryId){
        return Venue.builder().id(request.getId()).name(request.getName()).address1(request.getAddress1()).address2(request.getAddress2()).
