...
public class CountryRequest {
    ...
    public static Country from(CountryRequest request){
        return Country.builder().iso3(request.getIso3().toUpperCase()).iso2(request.getIso2().toUpperCase()).name(request.getName()).lat(request.getLat()).lng(request.getLng()).build();
    }
